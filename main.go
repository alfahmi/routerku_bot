...
func main() {
	...
	for update := range updates {
		...
		if(strings.Contains(update.Message.Text, "/start")){
			msg = tgbotapi.NewMessage(update.Message.Chat.ID, "Welcome to @routerku_bot!\n\nYour ChatID is:\n<b>" + strconv.Itoa(int(update.Message.Chat.ID)) + "</b>\n\nIf you want to know a little more about this bot, send:\n/about\n\nGreetings, alfahmi")
		}else if(strings.Contains(update.Message.Text, "/about")){
			msg = tgbotapi.NewMessage(update.Message.Chat.ID, "@routerku_bot was built by alfahmi and its source code is open sourced on github.com/alfahmi/routerku_bot. Currently, @routerku_bot serves you from Heroku :)")
		}else{
